...
<head>
	...
	<?php
		...
		$results = DB::query("SELECT * FROM sisa WHERE jenis=8 and code=%s and company=%s" , $row['code'], $row['company']);
		$sextra = 0;
	...
	function validateForm(){
		...
		var sextra = document.myForm.sextra.value;
		if(uses > amount){
			alert("Use must be smaller than amount");
			return false;
		}
		document.myForm.extra.value = amount - uses - sextra;
	}	
	</script>
</head>
<body>
    <ul id='fl_table'>
	<form name="myForm" action="solarU.php" method="post" onsubmit="return validateForm()">
		<input type="hidden" name="sextra" value=<?= $sextra?>>	
	...
	<a href='sisaCreate.php?jenis=8&id=<?= $id ?>&code=<?= $row['code'] ?>&company=<?= $row['company'] ?>'><i class="icon-plus-sign icon-4x"></i></a>
